...
This project was developed using Python 3.9 and PyCharm, but the codebase is fully IDE-independent.
...
## 6. Configure Environment Variable (.env file)

   A sample configuration file is includes as: .env.example
   
   Create your actual env file by copying it:
   ...
   (Yoy may customize SECRET_KEY if needed.)
   
## 7. Initialize the Database
   HydrateTrack uses SQLite and automatically creates hydration.db on first run.
   Run:
   	
	python app.py
   
   If resetting the database is needed/ deleting:
...
At this point, the application becomes fully functional with no additional configuration.
